...
const BOOKINGS_URL = "/consalting/bookings/"; // GET (page_size=1000), фильтр по employee на клиенте
...
function ConsultingSchoolTeachers() {
  ...
  const fetchBookings = useCallback(async (employeeId) => {
    ...
    setBErr("");
    try {
      const { data } = await api.get(BOOKINGS_URL, { params: { page_size: 1000 } });
  ...
  const fetchSalaries = useCallback(async (pageNum, employeeId) => {
    ...
    try {
      ...
      const rows = asArray(data);
      setSItems(rows);
      setSCount(typeof data?.count === "number" ? data.count : rows.length);
      setSPage(pageNum);
    } catch (err) {
      console.error(err);
  ...
  const fetchSales = useCallback(async (pageNum, employeeId) => {
    ...
    try {
      ...
      setSlItems(rows);
      setSlCount(typeof data?.count === "number" ? data.count : rows.length);
      setSlPage(pageNum);
    } catch (err) {
      console.error(err);
  ...
  const submitEmployeeCreate = async (e) => {
    ...
    if (isEmailDuplicate(email)) return setEmpErr("Сотрудник с таким e-mail уже существует.");
    if (!isHumanName(first_name) || !isHumanName(last_name))
      return setEmpErr("Имя и Фамилия: 2–60 символов (буквы, пробел, ' -).");
    if (!roleChoiceKeys.has(roleChoice)) return setEmpErr("Выберите доступную роль.");
    if (pctParsed === null) return setEmpErr("Процент должен быть числом от 0 до 100.");
  ...
  const submitEmployeeEdit = async (e) => {
    ...
    if (!roleChoiceKeys.has(roleChoice)) return setEmpEditErr("Выберите доступную роль.");
    if (pctParsed === null) return setEmpEditErr("Процент должен быть числом от 0 до 100.");
  ...
  const removeEmployee = async (u) => {
    ...
    if (
      !window.confirm(
        `Удалить сотрудника «${fullName(u) || u.email || "—"}»? Действие необратимо.`
      )
    )
      return;
  ...
  return (
    <div className="Schoolteachers">
          ...
          >
            ...
            >
              <div className="Schoolteachers__formGrid">
                ...
                <div className="Schoolteachers__field Schoolteachers__field--full">
                  <label className="Schoolteachers__label">
                    Процент от продажи (%) <span className="Schoolteachers__req">*</span>
                  </label>
                  <input
                    type="number"
                    className="Schoolteachers__input"
      ...
      {histOpen && (
        ...
        >
          <div className="Schoolteachers__modal" onClick={(e) => e.stopPropagation()}>
            <div className="Schoolteachers__modalHeader">
            ...
            <div className="Schoolteachers__tabs Schoolteachers__tabs--history">
              <button
                type="button"
                className={`Schoolteachers__tab ${histTab === "bookings" ? "is-active" : ""}`}
              ...
              <button
                type="button"
                className={`Schoolteachers__tab ${histTab === "salary" ? "is-active" : ""}`}
              ...
              <button
                type="button"
                className={`Schoolteachers__tab ${histTab === "sales" ? "is-active" : ""}`}
            ...
            {histTab === "bookings" && (
              <>
                ...
                <div className="Schoolteachers__search" style={{ marginTop: 8 }}>
                  <FaSearch className="Schoolteachers__searchIcon" aria-hidden />
                  <input
                    className="Schoolteachers__searchInput"
                  ...
                  {bLoading ? (
                    ...
                    bRows.map((it) => (
                      <article key={it.id} className="Schoolteachers__card">
                        <div>
                          <p className="Schoolteachers__name">
                            <b>{it.date || "—"} • {toHM(it.time) || "--:--"}</b> — {it.title || "—"}
                          </p>
                          {it.note && (
                            <div className="Schoolteachers__meta">
                              <span>{it.note}</span>
                  ...
                  {sLoading ? (
                    ...
                    sFiltered.map((it) => (
                      ...
                      >
                        <div>
                          <p className="Schoolteachers__name">
                            Сумма: <b>{money(it.amount)}</b> • Процент: <b>{String(it.percent || "")}</b>
                          </p>
                          {it.description && (
                            <div className="Schoolteachers__meta">
                ...
                {sCount > S_PER_PAGE && (
                  <div className="Schoolteachers__pager">
                    <button
                      type="button"
                      className="Schoolteachers__btn Schoolteachers__btn--secondary"
                      onClick={() => fetchSalaries(Math.max(1, sPage - 1), histEmp.id)}
                      disabled={sPage <= 1}
                    >
                      <FaChevronLeft /> Пред
                    </button>
                    <span className="Schoolteachers__page">
                      Стр. {sPage} из {Math.max(1, Math.ceil(sCount / S_PER_PAGE))}
                    </span>
                    <button
                      type="button"
                      className="Schoolteachers__btn Schoolteachers__btn--secondary"
                      onClick={() =>
                        fetchSalaries(
                          Math.min(Math.max(1, Math.ceil(sCount / S_PER_PAGE)), sPage + 1),
                          histEmp.id
                        )
                      }
                      disabled={sPage >= Math.max(1, Math.ceil(sCount / S_PER_PAGE))}
                    >
                      След <FaChevronRight />
                    </button>
                  </div>
            ...
            {histTab === "sales" && (
              <>
                {!!slErr && <div className="Schoolteachers__alert">{slErr}</div>}
                  ...
                  {slLoading ? (
                    ...
                    slFiltered.map((it) => (
                      <article key={it.id} className="Schoolteachers__card">
                        <div>
                          <p className="Schoolteachers__name">
                            <b>{toYMDhm(it.created_at) || "—"}</b> — {it.service_display || "—"} • {money(it.service_price)} сом
                          </p>
                          <div className="Schoolteachers__meta">
                            <span>Клиент: {it.client_display || "—"}</span>
                            {it.description ? (<><span>•</span><span>{it.description}</span></>) : null}
                          </div>
                        </div>
                      </article>
                    ))
                ...
                {/* {slCount > SL_PER_PAGE && (
                  <div className="Schoolteachers__pager">
                    <button
                      type="button"
                      className="Schoolteachers__btn Schoolteachers__btn--secondary"
                      onClick={() => fetchSales(Math.max(1, slPage - 1), histEmp.id)}
                      disabled={slPage <= 1}
                    >
                      <FaChevronLeft /> Пред
                    </button>
                    <span className="Schoolteachers__page">
                      Стр. {slPage} из {Math.max(1, Math.ceil(slCount / SL_PER_PAGE))}
                    </span>
                    <button
                      type="button"
                      className="Schoolteachers__btn Schoolteachers__btn--secondary"
                      onClick={() =>
                        fetchSales(
                          Math.min(Math.max(1, Math.ceil(slCount / SL_PER_PAGE)), slPage + 1),
                          histEmp.id
                        )
                      }
                      disabled={slPage >= Math.max(1, Math.ceil(slCount / SL_PER_PAGE))}
                    >
                      След <FaChevronRight />
                    </button>
                  </div>
